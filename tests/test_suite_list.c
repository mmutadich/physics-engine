...
void test_list_small() {
  ...
  *(vector_t *)list_get(l, 3) =
      (vector_t){7, 7}; // what do we do about this part??
...
void test_out_of_bounds_access() {
  ...
  for (size_t size = 0; size <= max_size; size++) {
    ...
    for (access->index = 0; access->index < size; access->index++) {
      ...
      *(vector_t *)list_get(access->list, access->index) =
          new_vector; // this is strange??
...
void remove_from_empty(void *l) { list_remove_back((list_t *)l); }
